...
class Staticman {
  ...
  _checkForSpam (fields) {
    // By default, Akismet is disabled in both configs.
    if (!config.get('akismet.enabled') && !this.siteConfig.get('akismet.enabled')) {
      console.log('Akismet processing disabled.');
    ...
    return new Promise((resolve, reject) => {
      ...
      if (akismetBypassValue !== null && akismetBypassValue !== '' && typeof akismetBypassValue !== 'undefined') {
        if (akismetData.comment_author === akismetBypassValue ||
          ...
          console.log('Akismet processing bypassed.');
  ...
  _validateConfig (config) {
    ...
    /*
     * Do not require the "branch" property in the site/repo config, as there is no one value that 
     * will be able to be promoted without change through multiple environments (e.g., dev, 
     * staging, prod). In such a set-up, setting the "branch" property, instead, in the server 
     * config (via environmental variables) makes more sense.
     */
  ...
  getSiteConfig (force) {
    ...
    return this.git.readFile(this.configPath.file).then(data => {
      const repoConfig = objectPath.get(data, this.configPath.path)
      const validationErrors = this._validateConfig(repoConfig)
      ...
      /*
       * If the allowed branch is specified in config.js, make sure that the branch specified in 
       * the request matches. If no branch is specified in config.js, do the same check against the 
       * value in the site/repo configuration.
       */
      const serverConfigBranch = config.get('branch')
      if (serverConfigBranch && (serverConfigBranch !== this.parameters.branch)) {
        return Promise.reject(errorHandler('BRANCH_MISMATCH'))
      } else if (repoConfig.branch && (repoConfig.branch !== this.parameters.branch)) {
        return Promise.reject(errorHandler('BRANCH_MISMATCH'))
      }
  ...
  processEntry (fields, options) {
    ...
    return this.getSiteConfig().then(config => {
      ...
      let commitMessage = this._resolvePlaceholders(this.siteConfig.get('commitMessage'), {
        fields,
        options
      })
      const exeEnv = config.get('exeEnv')
      ...
      if (exeEnv && (exeEnv !== exeEnvProd)) {
        // Identify the source environment to flag/prevent cross-talk between environments.
        commitMessage = exeEnv + ' - ' + commitMessage
      }
